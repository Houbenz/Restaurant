<hr class="hr my-5">    

@include('messages.create')

<footer class="page-footer font-small unique-color-dark my-4">

    <div class="bg-primary py-3">
        <div class="container  text-center">
            <div class="row text-center">
                <div class="col-md-3"></div>
                <div class="col-md-6">
                    <h5 class="text-center text-white">Get Connected with us in social media</h5>
                </div>
                <div class="col-md-3"></div>
            </div>

        </div>
        <div class=" container mt-4">
                <div class="row text-center">
       
                
                <div class="col-md-3"></div>
                
                <div class="col-md-6">
                    <i>
                        <img src="{{ asset('pictures/facebook.svg')}}" alt="facebook" width="64" height="64" class="mr-5" >
                    </i>
                    <i>
                        <img src="{{ asset('pictures/instagram.svg')}}" alt="instagram" width="64" height="64" class="mr-5" >
                    </i>
                    <i>
                        <img src="{{asset('pictures/twitter.svg')}}" alt="facebook" width="64" height="64" >
                    </i>
                </div>

                <div class="col-md-3"></div>
                </div>

            </div>
        </div>

        <div class="bg-dark">
            <div class="container pt-4 mb-4 text center">
                <div class="row mt-3">
                    <div class="col-md-3"></div>
                    <div class="col-md-3 text-white text-center mb-3">
                        <h3 class=" pb-2 text-center text-white 
                            text-uppercase text-footer-border"><strong>
                                <span class="text-warning">resta</span><span class="text-primary">urant</span></strong> &trade;</h3>

                            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Molestiae eligendi eum voluptatum hic quae, repudiandae ipsa,
                                 sit cumque nisi quam reprehenderit nulla accusantium nesciunt quibusdam maiores numquam magni tempora. Quis!</p>
                    </div>
                    
                    <div class="col-md-3 text-white text-center mb-3">

                        
                            <h3 class="pb-2 text-center text-white 
                            text-uppercase text-footer-border"><strong>contact</strong></h3>
                            <ul style="list-style-type: none">
                                <li>Khroub constantine</li><br>
                                <li>restaurant@gmail.com</li><br>
                                <li>+213 031807737</li><br>
                                <li>+213 0794511326</li>
                            </ul>
                    </div>
                    
                    <div class="col-md-3"></div>

                </div>
            </div>
            
        </div>

</footer>

